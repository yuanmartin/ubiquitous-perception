...
{	
	...
	return model_load(strPath,m_rkctFaceDetect,output_size);
}

void FaceDetection::Run(const Mat &srcImg,const Mat& midImg,const int& s32ReWidth,const int& s32ReHeight,vector<FaceRectInfo>& vecFaceRect)
{	
	//def input
	rknn_input inputs[input_size];
	model_input(inputs,midImg.rows * midImg.cols * img_channels,midImg.data);

	//model output
	rknn_output outputs[output_size];
	int ret = model_output(inputs,m_rkctFaceDetect,outputs,output_size);
	if(0 > ret)
	{
		return;
	}
	...
	//output transform
	OutputTrans(outputs,srcImg,midImg,s32ReWidth,s32ReHeight,vecFaceRect);

	//release resource
	rknn_outputs_release(m_rkctFaceDetect, output_size,outputs);
}

void FaceDetection::OutputTrans(rknn_output outputs[],const Mat& srcImg,const Mat& midImg,const int& s32ReWidth,const int& s32ReHeight,vector<FaceRectInfo>& vecFaceRect)
{
	// Post Process
	...
	DetectOutput detectoutput(conf_data_length / 2, midImg.cols, midImg.rows);
	int ret = detectoutput.Output(loc_data, conf_data,faceinfos, GetThreshold());
    if (0 == ret) 
	{
		vector<FaceRectInfo> vecEmpty;
		vecFaceRect.swap(vecEmpty);
		...
		for (int i = 0; i < faceinfos.size(); ++i) 
		{
			...
			if (0 < s32ReHeight && 0 < s32ReWidth) 
			{
				face_item.bbox.x = int(face_item.bbox.x * (srcImg.cols * 1.0f / s32ReWidth));
				face_item.bbox.y = int(face_item.bbox.y * ( srcImg.rows * 1.0f / s32ReHeight));
				face_item.bbox.width = int(face_item.bbox.width * (srcImg.cols * 1.0f / s32ReWidth));
				face_item.bbox.height = int(face_item.bbox.height * (srcImg.rows * 1.0f / s32ReHeight));
			}

			face_item.score = faceinfos[i][2];
			vecFaceRect.push_back(face_item);
		}
	}
}
...
int DetectOutput::Output(const float* loc_data, const float* conf_data,vector<vector<float> >& outputs, const float face_thresh) 
  {
	...
	std::vector<int> indices;
	ApplyNMSFast(b_bboxes, class1_confs, conf_thresh, nms_thresh, top_k,&indices);
	...
	std::stable_sort(idx_scores.begin(), idx_scores.end(),this->SortScorePairDescend);
	if (det_num > top_k) 
	{
		det_num = top_k;
	}
