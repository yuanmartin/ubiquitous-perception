...
namespace Vision_FaceAlg
{
    ...
    struct FaceRect 
    {
        ...
        FaceRect(const int &x_,const int &y_,const int &width_,const int &height_)
        ...
        void ResizeRect(const float& scale)
        {
            x *= scale;
		    y *= scale;
		    width *= scale;
    ...
    struct FaceRectInfo 
    {
        ...
        FaceRectInfo(const FaceRect &face_rects,const float &score_,const cv::Mat &landmarks_): bbox(face_rects), score(score_) 
        {
    ...
    public:
        ...
        void Run(const cv::Mat &srcImg,const cv::Mat& midImg,const int& s32ReWidth,const int& s32ReHeight,std::vector<FaceRectInfo>& vecFaceRect);
        
    private:

        bool Preprocess(const cv::Mat& srcImg,cv::Mat& dstImg,int& s32Width,int& s32Height);

        void OutputTrans(rknn_output outputs[],const cv::Mat& srcImg,const cv::Mat& dstImg,const int& s32ReWidth,const int& s32ReHeight,std::vector<FaceRectInfo>& vecFaceRect);
