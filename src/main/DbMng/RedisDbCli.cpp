...
bool CRedisDbCli::Init()
{
	...
	for (auto elm : vecSaveRedisCfg)
	{
		...
		//消息订阅
		string&& strDBIdx = string(elm[strAttriType]);
		string&& strSaveTopic = SaveRedisDBTopic(strDBIdx);
