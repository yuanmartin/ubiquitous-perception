...
bool CAppModuleMng::StartAppModule()
{
	//获取应用配置
	Json AppCfg;
	if(!SCCfgMng.GetRuntimeSysCfg(strAppNode,AppCfg))
	{
		return false;
	}
	...
	//配置工具的tcpsvr通信配置
	Json SouthTcpSvrCfg;
	if(!SCCfgMng.GetRuntimeSysCfg(strTransferNode + strTransferSouthTcpSvrNode,SouthTcpSvrCfg))
	{
		return false;
	}
...
void CAppModuleMng::GetAppModuleTaskLst(const tuple<string,Json>& taskCfg)
{
	...
	for(auto jTask : jTaskLst)
	{
		...
		//算法配置
		Json algCfg;
		if(!SCCfgMng.GetRuntimeSysCfg(AlgCfgMap[string(jTask[strAttriAlgIdx])],algCfg))
		{
			return;
		}
		...
		{
			SCCfgMng.GetRuntimeSysCfg(string(elm[strAttriTransferId]),vecDataSrcCfg);
		}
