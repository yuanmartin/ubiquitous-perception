...
CHttpClient::CHttpClient(const string& strAuth,const string& strHost,const int& s32Port) :
m_strHost(strHost),
m_strPort(to_string(s32Port)),
...
{
    //构建整个包发送头
    BuildHeard(strPage, strContentType,strReqType,ostreamReqPkg);
...
{
    //构建整个包发送头
    BuildHeard(strPage, strMultiPartContentType,"POST",ostreamReqPkg);
...
void CHttpClient::BuildHeard(const string& strPage,const string& strContentType,const string& strReqType,ostream& ostreamHeard)
{
    //构建发送头(请求行 + 请求头部)
    ostreamHeard << strReqType << " " << strPage << " HTTP/1.1" << "\r\n";                      //请求行
    ostreamHeard << "Host: " << m_strHost << ":" << m_strPort << "\r\n";
    ostreamHeard << "Authorization: " << m_strAuthorization << "\r\n";
    ostreamHeard << "Accept: */*\r\n";
