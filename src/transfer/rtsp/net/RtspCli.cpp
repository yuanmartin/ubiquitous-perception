...
void CRtspCli::Open(UsageEnvironment& env, char const* rtspURL, TCameraBaseInfo& tCamBaseInfo, FrameCallBackFunc CallBackFunc)
{
	m_pOurRtspClient = ourRTSPClient::createNew(env, rtspURL, RTSP_CLIENT_VERBOSITY_LEVEL, tCamBaseInfo.strCameraCode.c_str());
	if (m_pOurRtspClient == NULL) 
    {
		return;
	}

	m_pOurRtspClient->m_CallBackFunc = CallBackFunc;
	SetCameraBaseInfo(tCamBaseInfo, m_pOurRtspClient->m_CamBaseInfo);

	Authenticator Auth(tCamBaseInfo.strUserName.c_str(), tCamBaseInfo.strPassword.c_str());
	m_pOurRtspClient->sendDescribeCommand(continueAfterDESCRIBE, &Auth);
}
